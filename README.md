...
```console
$ runproof doctor
  ✗ the scheduler last ticked 8 hours ago and is presumed dead. 1 of 1 schedule is overdue.
```
...
| [`dashboard.py`](src/runproof/dashboard.py) | one self-contained HTML file: what has been happening over a week |
...
```bash
runproof dashboard          # writes .runproof/dashboard.html
```

One file. Inline CSS, no script, no CDN, nothing fetched — so it opens on a
plane, and it is safe to drag straight into a bug report, which is the form
this information usually needs to travel in.

**[See a sample →](docs/dashboard.html)** ([rendered](https://htmlpreview.github.io/?https://github.com/CAOShurong/runproof/blob/main/docs/dashboard.html)).
It is generated by `docs/build_docs.py` from a store written through the real
API with supplied timestamps, and CI fails if the committed copy drifts —
same rule as the figures.

What it shows, and why each part is there:

- **"Is anything not happening?"** first, above the results. A job whose
  schedule stopped firing appears here even though it has produced nothing to
  put in a table, and so does a run that was dispatched and then went quiet.
  Every dashboard that has ever misled anybody did it by having nothing to
  draw and drawing nothing.
- **Pass rate per job, over every attempt ever recorded** — not the last
  result. A job at 9/10 and a job at 1/10 both "worked last night".
- **A tick per attempt**, oldest to newest. Deliberately not a line chart:
  each attempt either passed or it did not, and a line through booleans
  invents a trend between points with nothing between them.
...
runproof dashboard             write a self-contained HTML summary
```
